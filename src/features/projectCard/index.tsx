...
export default async function ProjectCard(props: Props) {
  return (
    <div>
      <div className="relative group overflow-hidden shadow-xl w-full text-gray-800">
        <img src={props.imgSrc} alt="" className="object-cover h-full w-full" />
      ...
      <div className="flex flex-col p-2 sm:p-4 sm:gap-2">
        <h2 className="font-bold sm:text-2xl">{props.title}</h2>
        <p className="sm:text-lg">{props.technologies.join(", ")}</p>
      </div>
    </div>
  );
}
